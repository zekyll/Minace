...
private:
	...
	TTEST_CASE("Can do en passant.")
	{
		...
		s.makeMove("d2-d4");
		TTEST_EQUAL(ai->getMove(s).toStr(), "e4xd3");
	}
	...
	TTEST_CASE("Bugfix test #1.")
	{
		...
		TTEST_EQUAL(ai->getMove(s).toStr(), "Qd3-a6");
	}
	...
	TTEST_CASE("Bugfix test #2.")
	{
		...
		TTEST_EQUAL(ai->getMove(s).toStr(), "Kb4-a3");
	}
};
